...
#if defined(NDEBUG)
#error "EXPi cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 8 * 60 * 60; //8 hours RELEASE
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    if (tx.ContainsZerocoins())
        return state.DoS(10, error("AcceptToMemoryPool : Zerocoin transactions are disabled at EXPi"), REJECT_INVALID, "bad-tx");
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zEXPi spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zEXPi spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zEXPi spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 1) {
        nSubsidy = 90000 * COIN;  //premine 90,000
    } else if(nHeight >=2 && nHeight <= Params().LAST_POW_BLOCK()) { //PoW phase
        nSubsidy = 0.2 * COIN;
    } else if(nHeight >Params().LAST_POW_BLOCK() && nHeight < 20000) { //PoS phase
        nSubsidy = 0.75 * COIN;
        ...
        if(nMul < 32)
			nSubsidy = (0.2 / pow(2,nMul)) * COIN;
		else
			nSubsidy = 0; //down to zero
    } else {
        nSubsidy = 0.2 * COIN;  //everything else
    }
    return nSubsidy;
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from EXPi
...
{
    RenameThread("expi-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZEXPiMinted()
{
...
void RecalculateZEXPiSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zEXPi supply
...
bool RecalculateEXPiSupply(int nHeightStart)
{
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (tx.ContainsZerocoins())
            return state.DoS(100, error("ConnectBlock() : zerocoin transactions are disabled at EXPi"));
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zEXPi with serial %s is already in the block %d\n",
    ...
    //LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zEXPiSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert EXPi to zEXPi
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zEXPi spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zEXPi serial %s in block\n Block: %s",
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
		...
		if (!chainActive.Contains(pindexPrev) && pindexPrev != NULL) {
			...
			//check no more than 100 blocks in depth
			while (nCount < 100 && last != NULL && !chainActive.Contains(last)) {
				CBlock bl;
			...
			if(nCount >= 100)
				return false; // too many blocks out of main chain
		}
    }
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zEXPi mints and %d zEXPi spends\n", __func__, nMints, nSpends);
...
bool LoadExternalBlockFile(FILE* fileIn, CDiskBlockPos* dbp)
{
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                // locate a header
                unsigned char buf[MESSAGE_STEXPi_SIZE];
                blkdat.FindByte(Params().MessageStart()[0]);
                nRewind = blkdat.GetPos() + 1;
                blkdat >> FLATDATA(buf);
                if (memcmp(buf, Params().MessageStart(), MESSAGE_STEXPi_SIZE))
                    continue;
                // read size
                blkdat >> nSize;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // EXPi: We use certain sporks during IBD, so check to see if they are
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        if (memcmp(msg.hdr.pchMessageStart, Params().MessageStart(), MESSAGE_STEXPi_SIZE) != 0) {
            LogPrintf("PROCESSMESSAGE: INVALID MESSAGESTEXPi %s peer=%d\n", SanitizeString(msg.hdr.GetCommand()), pfrom->id);
            fOk = false;
            break;
        }
