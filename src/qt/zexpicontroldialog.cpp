...
#include "ui_zexpicontroldialog.h"
...
std::list<std::string> ZEXPiControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZEXPiControlDialog::listMints;

ZEXPiControlDialog::ZEXPiControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZEXPiControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZEXPiControlDialog::~ZEXPiControlDialog()
{
    delete ui;
}

void ZEXPiControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZEXPiControlDialog::updateList()
{
...
void ZEXPiControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZEXPiControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZEXPi_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZEXPiControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZEXPiControlDialog::GetSelectedMints()
{
...
void ZEXPiControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
