                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zEXPi ought to be enough for anybody." - Bill Gates, 2017
    ui->zEXPipayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzEXPiSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzEXPi->setEnabled(false);
        ui->pushButtonMintzEXPi->setToolTip(tr("zEXPi is currently disabled due to maintenance."));

        ui->pushButtonSpendzEXPi->setEnabled(false);
        ui->pushButtonSpendzEXPi->setToolTip(tr("zEXPi is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zEXPipayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzEXPi_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zEXPi is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zEXPi...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zEXPi in ") +
...
void PrivacyDialog::on_pushButtonSpendzEXPi_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zEXPi is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zEXPi
        sendzEXPi();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zEXPi
    sendzEXPi();
}

void PrivacyDialog::on_pushButtonZEXPiControl_clicked()
{
    ZEXPiControlDialog* zEXPiControl = new ZEXPiControlDialog(this);
    zEXPiControl->setModel(walletModel);
    zEXPiControl->exec();
}

void PrivacyDialog::setZEXPiControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzEXPiSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzEXPi()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid EXPi Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zEXPipayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zEXPi is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " EXPi </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zEXPi</b>";
    ...
    // use mints from zEXPi selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZEXPiControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZEXPiControlDialog::GetSelectedMints();
    }
    ...
    // Clear zexpi selector in case it was used
    ZEXPiControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zEXPi Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " EXPi, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zEXPi Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zEXPi </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zEXPi "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zEXPi "));
    ui->labelzEXPiAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzEXPiSyncStatus->setVisible(fShow);
}
