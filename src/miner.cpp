...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zEXPi spends in this block or tx
            if (tx.IsZerocoinSpend()) {
                int nHeightTx = 0;
                ...
                //This zEXPi serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("EXPiMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("EXPiMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("EXPiMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("expi-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running EXPiMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadEXPIMiner(void* parg)
{
    boost::this_thread::interruption_point();
    ...
    try {
        ...
        LogPrintf("ThreadEXPIMiner() exception");
    } catch (...) {
        LogPrintf("ThreadEXPIMiner() exception");
    }

    LogPrintf("ThreadEXPIMiner exiting\n");
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet, int nThreads)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ThreadEXPIMiner, pwallet));
}
